...
Data structure
 By: Julien Barbier
 Weight: 1
 Project over - took place from Nov 30, 2022 6:00 AM to Dec 2, 2022 6:00 AM
...
OBOBOB==3843==   total heap usage: 9 allocs, 9 frees, 1,152 bytes allocated
OBOBOB==3843== 
==3843== All heap blocks were freed -- no leaks are possible
==3843== 
OBOBOB==3843== For counts of detected and suppressed errors, rerun with: -v
OBOBOB==3843== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)
julien@ubuntu:~/0x13. More singly linked lists$ 
OBOBOBRepo:

GitHub repository: alx-low_level_programming
Directory: 0x13-more_singly_linked_lists
OBOBOBFile: 5-free_listint2.c
...
#include <stdlib.h>
OAOAOA#include <string.h>
#include <stdio.h>
OAOAOA#include "lists.h"

/**
OAOAOA * main - check the code
OAOAOA *
 * Return: Always 0.
OAOAOAOAOAOA */
int main(void)
...
OAOAOA    int n;
    ...
    add_nodeint_end(&head, 1);
    add_nodeint_end(&head, 2);
OBOBOB    add_nodeint_end(&head, 3);
    add_nodeint_end(&head, 4);
OBOBOB    add_nodeint_end(&head, 98);
    add_nodeint_end(&head, 402);
    add_nodeint_end(&head, 1024);
OBOBOB    print_listint(head);
    n = pop_listint(&head);
    printf("- %d\n", n);
    print_listint(head);
    n = pop_listint(&head);
